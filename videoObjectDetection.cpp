...
/*
* Helper method to handle integer overflow while finding next while iterating through multiple objects.
*/
...
/*
* Helper method to handle integer overflow while finding previous while iterating through multiple objects.
*/
...
// Create a list of randomly generated colors to use in Task 3 to highlight objects
...
/*
* Helper method to accomplish Task 1
*/
void generateBinarizedImage()
{
    ...
    customfilters::greyscale(blurredImage, greyscaleImage);
    cv::Mat binarizedImage = cv::Mat::zeros(image.rows, image.cols, CV_8UC1);
    objectrecognition::generateBinaryImage(greyscaleImage, binarizedImage, 100);
    binarizedImage.copyTo(displayImage);
}

/*
* Helper method to accomplish Task 2
*/
void generateCleanBinarizedImage()
{
    ...
    objectrecognition::grassfireTransform(displayImage, dilatedImage);
    dilatedImage.copyTo(displayImage);
}

/*
* Helper method to accomplish Task 3
*/
...
/*
* Helper method to display the image with label annotations, bounding boxes and axis of least central moment.
*/
void viewAnnotatedImage(cv::Mat& target, std::vector<std::string> labels, std::vector<std::string> otherVal) {
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        if (cv::contourArea(contours[i]) > minArea) {
            ...
            if (!labels.empty()) {
                ...
                if (!label.empty()) {
                    cv::putText(target, label, rect.center, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(78, 255, 0), 2);
                }
                if (!otherVal.empty()) {
                    std::string val = otherVal.at(i);
                    if (!val.empty()) {
                        cv::putText(target, val, cv::Point2f(rect.center.x, rect.center.y + 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(20, 150, 150), 2);
                    }
                }
            }
        }
...
/*
* Helper method to accomplish Task 4
*/
void generateAnnotatedImage()
{
    ...
    viewAnnotatedImage(annotatedImage, labels, labels);
    annotatedImage.copyTo(displayImage);
}

/*
* Helper method to classify objects using Nearest Neighbor to accomplish Task 6
*/
...
/*
* Helper method to classify objects using Multi-class K Nearest Neighbor to accomplish Task 7
*/
void runKNearestNeighbors()
{
    matchfunctions::findAllContours(displayImage, contours, hierarchy);
    std::vector<std::string> labels, sumDistances;
    matchfunctions::kNearestNeighbor(contours, hierarchy, labels, sumDistances);
    cv::Mat annotatedImage;
    image.copyTo(annotatedImage);
    viewAnnotatedImage(annotatedImage, labels, sumDistances);
    annotatedImage.copyTo(displayImage);
}
